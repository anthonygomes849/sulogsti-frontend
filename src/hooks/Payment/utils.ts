...
export const loadLinxPaykitScript = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    ...
    const existingScript = document.querySelector(`script[src="${import.meta.env.VITE_APP_API_LINX_PAYMENT_URL}"]`);
    if (existingScript) {
    ...
    script.src = import.meta.env.VITE_APP_API_LINX_PAYMENT_URL;
    script.async = true;
    script.onload = () => {
      // clearTimeout(timeoutId);
      resolve();
  ...
  try {
    ...
    //   }
      ...
      const authenticationRequest: LinxAuthenticationRequest = {
        authenticationKey: import.meta.env.VITE_APP_API_LINX_AUTHENTICATION_KEY,
      };
