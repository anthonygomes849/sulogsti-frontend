...
const ProprietarioCargas: React.FC = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<IProprietarioCargas>();
  ...
  const onDelete = useCallback(async (rowId?: number) => {
    try {
      setLoading(true);
      const body = {
        id_proprietario_carga: rowId,
      };

      await api.post("/deletar/proprietariosCarga", body);
    ...
    <>
      ...
      {isRemove && (
        <ModalDelete
          ...
          onConfirm={() => onDelete(selectedRow?.id_proprietario_carga)}
          row={selectedRow?.razao_social}
        />
      )}

      {isView && (
        <Info data={selectedRow} title="Conhecer - Proprietario Cargas" onClose={() => setIsView(!isView)} />
      )}
      <div className="flex flex-col w-full h-screen bg-[#F5F5F5] p-5">
        <div className="flex items-center justify-between w-full mb-3">
          <div>
            <h1 className="text-2xl text-[#000000] font-bold">Proprietario Cargas</h1>
          </div>
          <div className="mr-14">
        ...
        <div className="flex w-[calc(100vh - 90px)]">
          <Grid
            ...
            filters={[]}
            pagination
            path="/listar/proprietariosCarga"
            onDelete={(data: IProprietarioCargas) => {
              setIsRemove(!isRemove);
              setSelectedRow(data);
            }}
            status={[]}
            onView={(data: IProprietarioCargas) => {
              setSelectedRow(data);
              setIsView(!isView);
              // openModal();
            }}
            onUpdate={(data: IProprietarioCargas) => {
              setSelectedRow(data);
              setIsEdit(!isEdit);
              openModal();
            }}
...
export default ProprietarioCargas;
